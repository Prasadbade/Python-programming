...
def main():
    ...
    if (len(argv)==2):
        ...
        if (argv[1] == "-u") or (argv[1] == "-U"):
            print("usage : This script is used for write names of dublicate file in the log file")
            exit()

    if (len(argv)==2):
        try:
            #Dir_Dublicate(argv[1])
            schedule.every(1).minute.do(Dir_Dublicate)
            while True:
